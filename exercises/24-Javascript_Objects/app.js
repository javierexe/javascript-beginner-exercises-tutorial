...
function addAllFamilyLuckyNumbers(anArray){
  ...
  family.members.forEach(element => {
    let arr = element.luckyNumbers
    let sum = 0;
    for (value of arr) {
        sum += value;
    }
    sumOfAllLuckyNumbers += sum
});
...
function changeLuckyNumber (persona,number) {
    persona.luckyNumbers.pop();
    persona.luckyNumbers.push(number);
    }
changeLuckyNumber(person,33);
